...
import { demoSuggestedFollow } from "../../../data.json"
...
export default function Following() {
	...
	const fetchVids = useCallback(
		async (skip?: number) => {
			try {
				...
				getFollowingVids(username!, skip).then(res => console.log(res))
				return res.data;
			} catch (err: any) {
				dispatch(
	...
	useEffect(() => {
		...
		async function fetchFunc() {
			const res = await fetchVids() || {videos:[]};
	...
	useEffect(() => {
		...
		async function fetchSuggestions() {
			try {
				...
				// console.log(demoSuggestedFollow)
				// setSuggestions(demoSuggestedFollow)
			}
		}
		fetchSuggestions();
